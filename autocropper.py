...
def segment_video(videoID, response):
    ...
    for i, segment in enumerate(response):
        ...
        output_file = f"{videoID}-{start_time}-{end_time}.mp4"
        output_file = output_file.replace('/', '_')  # Replace '/' with '_'
        command = f"ffmpeg -i input_video-{videoID}.mp4 -ss {start_time} -to {end_time} -c copy {output_file}"
        subprocess.call(command, shell=True)
    print("Video segmented successfully.")
...
def analyze_transcript(transcript):
    prompt = f"This is a transcript of a video. Please identify the  most viral section from the whole, make sure it is  more than 30 seconds in duration,Make Sure you provide extremely accurate timestamps respond only in this format, make sure it is valid json like not leaving a comma after the last item in the array, {response_obj}  \n Here is the Transcription:\n{transcript}"
    messages = [
        {"role": "system",
...
def AI_Cropper(video_id, number):
    ...
    # Extract the interesting segments from the transcript
    current_chunk = []
    ...
    for item in transcript:
        ...
        # If adding the current item exceeds the limit, start a new chunk
        if current_chunk_tokens + item_tokens > 5000:
            transcript_array.append(current_chunk)
            current_chunk = [item]
    ...
    print(f"Transcript split into {len(transcript_array)} chunks.")
    print("------------------")
    ...
    print("Printing interesting segments:")
    for seg in transcript_array:
        interesting_segment = analyze_transcript(seg)
        print("Found interesting segment:")
        content = interesting_segment.content
        parsed_content = json.loads(content.strip())
        print(parsed_content)
        # pdb.set_trace()
        segment_video(video_id, parsed_content)

    # Move input video and output segments to a new folder by "{yt.title}"
    os.mkdir(str(number))
    # move input video and any .mp4 files to the new folder
    os.system(f"mv *.mp4 {str(number)}/")
